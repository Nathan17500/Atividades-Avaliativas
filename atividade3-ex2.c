 ...
 *   gcc atividade3-ex2.c -o atividade3 -lraylib -lm -lpthread -ldl -lrt -lX11
 */
...
void desenharItem(Item *item) {
    ...
    if (item->tipo == ITEM_ESCUDO) {
        ...
        return;
    } else if (item->tipo == ITEM_ARMA) {
        DrawCircleV(item->pos, item->raio, RED);
    } else {
        if (item->dados.pocao.cura > 0) {
            DrawCircleV(item->pos, item->raio, GREEN);
        } else {
            DrawCircleV(item->pos, item->raio, DARKPURPLE);
        }
    }
}
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            DrawText("Vermelho = arma (aumenta dano) | Verde = pocao (cura) | Azul = armadura (aumenta defesa)", 10, 34, 18, GRAY);
