 ...
 *   gcc atividade1-ex2.c -o ex2 -lraylib -lm -lpthread -ldl -lrt -lX11
 *
 * Compilar (Windows, MinGW):
 *   gcc atividade1-ex2.c -o ex2.exe -lraylib -lgdi32 -lwinmm
 */
...
void desenharMatriz(int **matriz, int linhas, int colunas) {    
    for (int i = 0; i < linhas; i++) {
        for (int j = 0; j < colunas; j++) {
            Color cor = (matriz[i][j] == 1) ? (Color){140, 190, 255, 255}
                                             : (Color){15, 30, 55, 255};
...
void celulaVisitada (int **matriz, int linhas, int colunas, Bola *b) {
...
int atualizarContador (int **matriz, int linhas, int colunas) {
    int contador = 0;
    for (int i = 0; i < linhas; i++) {
        for (int j = 0; j < colunas; j++) {
            if (matriz[i][j] == 1) {
...
void adicionarBolas (Bola **b, int *quantidade) {
    if (IsKeyPressed(KEY_SPACE)) {
        *quantidade = *quantidade + 1;
        int q = *quantidade;
        *b = realloc (*b, q * sizeof(Bola));
        Bola *novaBola = (*b + (q-1));
            novaBola->pos = (Vector2){ GetRandomValue(50, LARGURA_JANELA - 50),
                                GetRandomValue(50, ALTURA_JANELA - 50) };
            novaBola->vel = (Vector2){ (float)GetRandomValue(-4, 4),
                                (float)GetRandomValue(-4, 4) };
            novaBola->raio = (float)GetRandomValue(10, 25);
            novaBola->cor  = (Color){ GetRandomValue(100,255), GetRandomValue(100,255),
                                GetRandomValue(100,255), 255 };
    }
}

void removerBolas (Bola **b, int *quantidade) {
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < quantidadeBolas; i++) {
            ...
            celulaVisitada(grade, linhas, colunas, bolas + i);
        }

        celulasVisitadas = atualizarContador(grade, linhas, colunas);

        adicionarBolas(&bolas, &quantidadeBolas);
        removerBolas(&bolas,&quantidadeBolas);
        ...
        BeginDrawing();
            ...
            snprintf(Contador, sizeof(Contador), "Celulas visitadas: %d", celulasVisitadas);
            DrawText (Contador, 10, 35, 18, WHITE);
